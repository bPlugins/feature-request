...
if (isset($_POST['settings_save_btn']) && check_admin_referer('my_settings_action', 'my_settings_nonce')) {
    ...
    $suggestion_settings = array(
        'sp_title_text' => isset($_POST['sp_title_text']) ? sanitize_text_field(wp_unslash($_POST['sp_title_text'])) : '',
        'sp_textarea_text' => isset($_POST['sp_textarea_text']) ? sanitize_textarea_field(wp_unslash($_POST['sp_textarea_text'])) : '',
        'sp_drop_text' => isset($_POST['sp_drop_text']) ? sanitize_text_field(wp_unslash($_POST['sp_drop_text'])) : '',
        'sp_email_text' => isset($_POST['sp_email_text']) ? sanitize_email(wp_unslash($_POST['sp_email_text'])) : '',
        'sp_submit_button_text' => isset($_POST['sp_submit_button_text']) ? sanitize_text_field(wp_unslash($_POST['sp_submit_button_text'])) : '',
        'sp_tab_title_text' => isset($_POST['sp_tab_title_text']) ? sanitize_text_field(wp_unslash($_POST['sp_tab_title_text'])) : '',
    );
    
    

    // Save Suggestion Form Settings
    update_option('fereq_suggestion_settings', $suggestion_settings);

    // Issues Form Settings
    $issues_settings = array(
        'ip_title_text' => isset($_POST['ip_title_text']) ? sanitize_text_field(wp_unslash($_POST['ip_title_text'])) : '',
        'ip_textarea_text' => isset($_POST['ip_textarea_text']) ? sanitize_textarea_field(wp_unslash($_POST['ip_textarea_text'])) : '',
        'ip_drop_text' => isset($_POST['ip_drop_text']) ? sanitize_text_field(wp_unslash($_POST['ip_drop_text'])) : '',
        'ip_email_text' => isset($_POST['ip_email_text']) ? sanitize_email(wp_unslash($_POST['ip_email_text'])) : '',
        'ip_submit_button_text' => isset($_POST['ip_submit_button_text']) ? sanitize_text_field(wp_unslash($_POST['ip_submit_button_text'])) : '',
        'ip_tab_title_text' => isset($_POST['ip_tab_title_text']) ? sanitize_text_field(wp_unslash($_POST['ip_tab_title_text'])) : '',
    );
    
    

    // Save Issues Form Settings
    update_option('fereq_issues_settings', $issues_settings);

    // Frontend Feedback Button Area Settings
    $feedback_button_settings = array(
        'fda_title_text' =>isset($_POST['fda_title_text']) ? sanitize_text_field(wp_unslash($_POST['fda_title_text'])): "",
        'feedback_button_text' =>isset($_POST['feedback_button_text']) ? sanitize_text_field(wp_unslash($_POST['feedback_button_text'])) : "",
    );

    // Save Frontend Feedback Button Area Settings
    update_option('fereq_feedback_button_settings', $feedback_button_settings);
...
$suggestion_settings = get_option('fereq_suggestion_settings', array(
...
$issues_settings = get_option('fereq_issues_settings', array(
...
$feedback_button_settings = get_option('fereq_feedback_button_settings', array(
