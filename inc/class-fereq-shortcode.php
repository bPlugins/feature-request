...
class fereq_Features_Request_Shortcode
{
    static $css = null;
    private $table_name;
    ...
    public function __construct()
    {
        ...
        add_shortcode("fereq_features-request", [$this, "features_request_shortcode_callback"]);
        ...
        add_action('wp_enqueue_scripts', [ $this, 'shortcode_css'] );
    ...
    public function features_request_shortcode_callback()
    {
        ...
        $dynamic_styles .= 'button.filter_button.active {
            background-color: ' . esc_attr($button_bg_color) . ' !important;
            color: ' . esc_attr($button_text_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'button.filter_button {
            background-color: ' . esc_attr($fereq_background_color) . ' !important;
            color: ' . esc_attr($filter_button_text_default) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'button.filter_tag.active {
            background-color: ' . esc_attr($button_bg_color) . ' !important;
            color: ' . esc_attr($button_text_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'button.filter_tag {
            background-color: ' . esc_attr($fereq_background_color) . ' !important;
            color: ' . esc_attr($filter_button_text_default) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'button.feedback_btn {
            background-color: ' . esc_attr($button_bg_color) . ' !important;
            color: ' . esc_attr($button_text_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'button.comment_btn {
            background-color: ' . esc_attr($button_bg_color) . ' !important;
            color: ' . esc_attr($button_text_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'button.vote_btn {
            background-color: ' . esc_attr($button_bg_color) . ' !important;
            color: ' . esc_attr($button_text_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'div.feedback_tab button[type="submit"] {
            background-color: ' . esc_attr($button_bg_color) . ' !important;
            color: ' . esc_attr($button_text_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'div.features_request_header_div {
            background-color: ' . esc_attr($fereq_background_color) . ' !important;
            color: ' . esc_attr($fereq_all_paragraph_font_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= '.features_request_body .tabs__content ul .filtering_data {
            background-color: ' . esc_attr($fereq_background_color) . ' !important;
            color: ' . esc_attr($fereq_all_paragraph_font_color) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'div.features_request_header_div .feedback_title {
            color: ' . esc_attr($fereq_all_title_font_color) . ' !important;
            font-size: ' . esc_attr($fereq_all_title_font_size) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'div.filtering_data_content h3 {
            color: ' . esc_attr($fereq_all_title_font_color) . ' !important;
            font-size: ' . esc_attr($fereq_all_title_font_size) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'div.filtering_data_content h3 span {
            color: #333 !important;
            font-size: inherit;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'div.filtering_data_content p {
            color: ' . esc_attr($fereq_all_paragraph_font_color) . ' !important;
            font-size: ' . esc_attr($fereq_all_paragraph_font_size) . ' !important;
        }' . PHP_EOL;
        
        $dynamic_styles .= 'span.upvote_icon, span.downvote_icon {
    ...
    public function shortcode_css(){
        wp_add_inline_style('fereq-frontend-css', self::$css );
    }
    ...
    public function issues_submitted_form_action_handler()
    {
        ...
        if (!empty($form_data)) {
            ...
            $form_array = array_map('sanitize_text_field', $form_array);
        } else {
            wp_send_json_error("Form data is missing or invalid.");
            return;
        }
    
        $issues_plugin_name = $form_array['issues_plugin_name'] ?? '';
        $issues_title = $form_array['issues_title'] ?? '';
        $issues_massage = $form_array['issues_massage'] ?? '';
        $issues_url = isset($form_array['issues_url']) && filter_var($form_array['issues_url'], FILTER_VALIDATE_URL) ? esc_url_raw($form_array['issues_url']) : '';
        $issues_email = isset($form_array['issues_email']) && is_email($form_array['issues_email']) ? sanitize_email($form_array['issues_email']) : '';
        $submit_privately = $form_array['submit_privately'] ?? '';
    
        if (empty($issues_title) || empty($issues_email)) {
            wp_send_json_error("Required fields are missing.");
            return;
        }
    
        $data = array(
            'report_type' => 'issues_submit',
            'report_plugin_name' => sanitize_text_field($issues_plugin_name),
            'report_title' => sanitize_text_field($issues_title),
            'report_description' => sanitize_textarea_field($issues_massage),
            'report_url' => esc_url_raw($issues_url),
            'report_email' => sanitize_email($issues_email),
            'report_status' => 'exploring',
            'submit_privately' => sanitize_text_field($submit_privately),
        );
    ...
    public function suggestion_submitted_form_action_handler()
    {
        ...
        if (!empty($form_data)) {
            ...
            // Sanitize all fields in the array
            $form_array = array_map('sanitize_text_field', $form_array);
        } else {
            wp_send_json_error("Form data is missing or invalid.");
            return;
        ...
        $suggestion_plugin_name = $form_array['suggestion_plugin_name'] ?? '';
        $suggestion_title = $form_array['suggestion_title'] ?? '';
        $suggestion_massage = $form_array['suggestion_massage'] ?? '';
        $suggestion_url = isset($form_array['suggestion_url']) && filter_var($form_array['suggestion_url'], FILTER_VALIDATE_URL) ? esc_url_raw($form_array['suggestion_url']) : '';
        $suggestion_email = isset($form_array['suggestion_email']) && is_email($form_array['suggestion_email']) ? sanitize_email($form_array['suggestion_email']) : '';
        $submit_privately = isset($form_array['submit_privately']) && in_array($form_array['submit_privately'], ['on', ''], true) ? $form_array['submit_privately'] : '';
        $report_type = 'suggestion_submit';
        $report_status = 'exploring';
    
        // Validate required fields
        if (empty($suggestion_title) || empty($suggestion_email)) {
            wp_send_json_error("Required fields are missing.");
            return;
        }
        ...
        $data = array(
            ...
            'report_plugin_name' => sanitize_text_field($suggestion_plugin_name),
            'report_title' => sanitize_text_field($suggestion_title),
            'report_description' => sanitize_textarea_field($suggestion_massage),
            'report_url' => $suggestion_url, // Already sanitized
            'report_email' => $suggestion_email, // Already sanitized
            'report_status' => sanitize_text_field($report_status),
            'submit_privately' => sanitize_text_field($submit_privately),
        );
        ...
        if (get_option("fereq_suggestion_customer_email_enable_disable", "") === "on") {
            $customer_email_subject = get_option("fereq_suggestion_customer_email_subject", "Thank you for your suggestion!");
            $customer_email_content = get_option("fereq_suggestion_customer_email_email_content", "");
            $customer_email_message = str_replace('{customer_email}', $suggestion_email, $customer_email_content);
    
            $headers = ['Content-Type: text/html; charset=UTF-8'];
            wp_mail($suggestion_email, $customer_email_subject, $customer_email_message, $headers);
        }
    
        // Handle admin email
        if (get_option("fereq_suggestion_enable_disable", "") === "on") {
            $admin_recipient = get_option("fereq_suggestion_recipient", get_option('admin_email'));
            $admin_email_subject = get_option("fereq_suggestion_subject", "New Suggestion Submitted");
            $admin_email_content = get_option("fereq_suggestion_email_content", "");
    
            $admins = get_users(['role' => 'administrator']);
            $admin_name = !empty($admins) ? $admins[0]->user_login : 'Admin';
    
            $admin_email_message = str_replace(
                ['{admin_name}', '{customer_email}', '{post_name}'],
                [$admin_name, $suggestion_email, $suggestion_title],
                $admin_email_content
            );
    
            wp_mail($admin_recipient, $admin_email_subject, $admin_email_message, $headers);
        }
...
$query = $this->wpdb->prepare("SELECT t1.*, COALESCE(subquery.total_agree_voting, 0) AS total_agree_voting, COALESCE(subquery.total_disagree_voting, 0) AS total_disagree_voting FROM {$table_name} AS t1 LEFT JOIN (SELECT report_table_id, SUM(CASE WHEN vote_ans = 'agree' THEN 1 ELSE 0 END) AS total_agree_voting, SUM(CASE WHEN vote_ans = 'disagree' THEN 1 ELSE 0 END) AS total_disagree_voting FROM {$table_name_2} GROUP BY report_table_id) AS subquery ON t1.id = subquery.report_table_id WHERE t1.report_status = 'done' AND t1.submit_privately != 'on'");
    ...
    public function popup_content_showing_after_click_action_handler()
    {
        ...
        $dataId = isset($_POST['dataId']) ? sanitize_text_field(wp_unslash($_POST['dataId'])) : "";
        $tabType = isset($_POST['tabType']) ? sanitize_text_field(wp_unslash($_POST['tabType'])) : "";
...
//phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                        ...
                        $cookieValueId = isset($_COOKIE['voting_cookie_set']) ? sanitize_text_field(wp_unslash($_COOKIE['voting_cookie_set'])) : '';
    ...
    public function voting_store_database_action_handler()
    {
        ...
        $email_value = isset($_POST['email_value']) ? sanitize_email(wp_unslash($_POST['email_value'])) : "";
        $textArea = isset($_POST['textArea']) ? sanitize_textarea_field(wp_unslash($_POST['textArea'])) : "";
        $select = isset($_POST['select']) ? sanitize_text_field(wp_unslash($_POST['select'])) : "";
        $report_table_id_val = isset($_POST['report_table_id_val']) ? sanitize_text_field(wp_unslash($_POST['report_table_id_val'])) : "";
        $keep_me_posted = isset($_POST['keep_me_posted']) ? sanitize_text_field(wp_unslash($_POST['keep_me_posted'])) : "";
    ...
    public function comment_store_database_action_handler()
    {
        ...
        $comment_email_field = isset($_POST['comment_email_field']) ? sanitize_text_field(wp_unslash($_POST['comment_email_field'])) : "";
        $comment_type = isset($_POST['comment_type']) ? sanitize_text_field(wp_unslash($_POST['comment_type'])) : "";
        $commentator_name = isset($_POST['commentator_name']) ? sanitize_text_field(wp_unslash($_POST['commentator_name'])) : "";
        $requestFrom = isset($_POST['requestFrom']) ? sanitize_text_field(wp_unslash($_POST['requestFrom'])) : "";
        $commentReplyMail = isset($_POST['commentReplyMail']) ? sanitize_text_field(wp_unslash($_POST['commentReplyMail'])) : "";
        $cTextarea = isset($_POST['cTextarea']) ? sanitize_textarea_field(wp_unslash($_POST['cTextarea'])) : "";
        $report_table_id_val = isset($_POST['report_table_id_val']) ? sanitize_text_field(wp_unslash($_POST['report_table_id_val'])) : "";
        $commentId = isset($_POST['commentId']) ? sanitize_text_field(wp_unslash($_POST['commentId'])) : "";
...
//phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    ...
    public function remove_vote_from_database_action_handler()
    {
        ...
        $report_table_id_val = isset($_POST['report_table_id_val']) ? sanitize_text_field(wp_unslash($_POST['report_table_id_val'])) : "";
        $voter_email = isset($_POST['voter_email']) ? sanitize_text_field(wp_unslash($_POST['voter_email'])) : "";
    ...
    public function edit_comment_data_action_handler()
    {
        ...
        $report_table_id_val = isset($_POST['reportId']) ? sanitize_text_field(wp_unslash($_POST['reportId'])) : "";
        $commentId = isset($_POST['commentId']) ? sanitize_text_field(wp_unslash($_POST['commentId'])) : "";
        $commenttype = isset($_POST['commenttype']) ? sanitize_text_field(wp_unslash($_POST['commenttype'])) : "";
        $comment_email = isset($_POST['comment_email']) ? sanitize_text_field(wp_unslash($_POST['comment_email'])) : "";        
    ...
    public function voting_email_checking_action_handler()
    {
        ...
        $report_table_id_val = isset($_POST['reportId']) ? sanitize_text_field(wp_unslash($_POST['reportId'])) : "";
        $voting_email = isset($_POST['voting_email']) ? sanitize_text_field(wp_unslash($_POST['voting_email'])) : "";
//phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    ...
    public function update_comment_data_action_handler()
    {
        ...
        $commenttype = isset($_POST['commenttype']) ? sanitize_text_field(wp_unslash($_POST['commenttype'])) : "";
        $comment_email = isset($_POST['comment_email']) ? sanitize_text_field(wp_unslash($_POST['comment_email'])) : "";
        $textareaValue = isset($_POST['textareaValue']) ? sanitize_text_field(wp_unslash($_POST['textareaValue'])) : "";
        $nameValue = isset($_POST['nameValue']) ? sanitize_text_field(wp_unslash($_POST['nameValue'])) : "";
        ...
        if ($update_result)
        {
            ...
            if($commentator_status){
                ...
                if($post_owner_status){
                    $get_comment_edit_admin_email_recipient = get_option("fereq_comment_delete_admin_email_recipient", "");
        ...
        if ($update_result)
        {
            ...
            if($commentator_status){
                ...
                if($post_owner_status){
                    ...
                    $get_comment_delete_admin_email_subject = get_option("fereq_fereq_comment_delete_admin_email_subject", "");
...
new fereq_Features_Request_Shortcode();